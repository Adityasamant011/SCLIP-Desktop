...
export function getNextShuttleRate(
  currentRate: number,
  ...
  const nextMagnitude =
    SHUTTLE_RATES.find((rate) => rate > currentMagnitude) ?? 4
  return direction * nextMagnitude
}

export function getBrowserMediaPlaybackRate(
  authoredRate: number,
  transportRate: number,
): number {
  const safeAuthoredRate =
    Number.isFinite(authoredRate) && authoredRate > 0 ? authoredRate : 1
  const safeTransportRate =
    Number.isFinite(transportRate) && transportRate > 0 ? transportRate : 1
